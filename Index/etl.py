...
def cleansing(df, column):
    ...
    dicc = {'["#]':'', "[-'\._()]":" ", '4K':'','Uhd':'', '&':'And', 'Seasons':'Season'}
    dicc_listed_in = {'["#/+&,]':'', "['\._()]":" ", 'And|Movies|Film|Series|Tv|Videos|Features|Young|Audience|Musicals':'', 'Shows':'Show', 'Thrillers':'Thriller', 'Concerts':'Concert', 'Dramas':'Drama', 'Classics':'Classic', 'Action-Adventure':'Action Adventure', 'Romantic':'Romance', 'Historical':'History', 'Documentaries|Docuseries':'Documentary', 'Musical':'Music', 'Comedies':'Comedy', 'Mysteries':'Mystery', 'Military.+War':'Military-War', 'Health.+Wellness': 'Health-Wellness', 'Science.+Fiction':'Sci-Fi', 'Black.+Stories':'Black-Stories', 'Late.+Night':'Late-Night', 'Coming.*Of.*Age':'Coming-Of-Age', 'Stand.+Up':'Stand-Up', 'Special.+Interest':'Special-Interest', 'Soap.*Opera.*Melodrama':'Soap-Opera-Melodrama'}    
...
df_title = pd.concat([df_amazon, df_disney, df_hulu, df_netflix], axis=0, ignore_index=True)
df_title = pd.concat([df_title, df_title['duration'].str.split(expand=True)], axis=1)
df_title.rename(columns={0:'duration_time', 1:'duration_unit'}, inplace=True)
df_title.drop(['show_id', 'type', 'director', 'country', 'date_added', 'rating', 'duration', 'description'], axis=1, inplace=True)
...
df_service = pd.DataFrame(['Amazon', 'Disney', 'Hulu', 'Netflix'], columns=['service'])
...
df_title_actor.drop(df_title_actor[(df_title_actor['actor'] == 'Sin Dato')].index, inplace=True)
df_title_actor = df_title_actor.merge(df_actor, how='left', on='actor')

df_title_listed_in = create_df_aux(df_title, 'title', 'listed_in', ' ')
df_title_listed_in = df_title_listed_in.merge(df_listed_in, how='left', on='listed_in')
...
df_title['duration_unit'].replace({'Sin Dato':'Min'}, regex=False, inplace=True)
df_title.drop_duplicates(['title', 'duration_unit'], keep='first', inplace=True)
df_title.reset_index(drop=True, inplace=True)
df_title.index.name = "id_title"
df_title.index += 1
df_title.reset_index(drop=False, inplace=True)
df_title['duration_time'].replace({'Sin Dato':None}, regex=False, inplace=True)
df_title['duration_time'] = df_title['duration_time'].astype('Int64')
df_title['duration_time'].fillna(int(df_title['duration_time'].median(skipna=True)), inplace=True)

df_title_actor = df_title_actor.merge(df_title[['id_title','title']], how='left', on='title')
df_title_actor['id_actor'] = df_title_actor['id_actor'].astype('Int64')
df_title_actor.drop(['title', 'actor'], axis=1, inplace=True)

df_title_listed_in = df_title_listed_in.merge(df_title[['id_title','title']], how='left', on='title')
df_title_listed_in.drop(['title', 'listed_in'], axis=1, inplace=True)

df_title_service = df_title_service.merge(df_title[['id_title','title']], how='left', on='title')
